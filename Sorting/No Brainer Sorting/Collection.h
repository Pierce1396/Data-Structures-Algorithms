...
void Collection<Item>::sortBySelection(){
    // Assumes the first element is the smallest number
    for(int i = 0; i < curSize; i++){
        Item smallest = findMinInRange(i);
        swap(smallest, i);
    }
    // compares that number with the next in  the list
    // if it finds one smaller, it swaps
    // once it goes to the end, it takes the next value in the list and then repeats the algorithm
